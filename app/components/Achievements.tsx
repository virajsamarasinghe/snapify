  ...
  useEffect(() => {
    ...
    const calculatePath = () => {
      ...
      dots.forEach((dot, index) => {
        ...
        pathString += ` C ${prevX + swing} ${prevY + (y - prevY) * 0.5}, ${x + swing} ${y - (y - prevY) * 0.5}, ${x} ${y}`;
              ...
              scrollTrigger: {
                  ...
                  onUpdate: (self) => {
                      ...
                      dots.forEach((dotWrapper, i) => {
                          ...
                          const pointAtLength = pathRef.current!.getPointAtLength(drawLength);
                          ...
                          if (pointAtLength.y >= dotTopY) {
                             ...
                             if (card && card.classList.contains('revealed')) {
                                ...
                                gsap.to(card, { opacity: 0, y: 50, scale: 0.95, duration: 0.5, ease: "power2.in" });
                             }
                          }
                      });
                  }
            ...
            {achievements.map((achievement, index) => (
              ...
              >
                ...
                <div className={`flex-1 ${index % 2 === 0 ? 'text-right' : 'text-left'}`}>
                  <span className="text-purple-400 font-mono text-xs tracking-widest block mb-2">{achievement.year}</span>
                  <h3 className="text-3xl lg:text-4xl font-bold text-white mb-4">{achievement.title}</h3>
                  <p className="text-white/40 max-w-sm ml-auto mr-0">{achievement.description}</p>
                </div>
