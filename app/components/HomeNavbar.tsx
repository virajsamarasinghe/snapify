...
export default function HomeNavbar({ animateOnMount = true, delay = 0, className = "" }: HomeNavbarProps) {
  ...
  return (
    <>
      <nav ref={navRef} className={`hero-nav fixed top-0 left-0 w-full px-8 lg:px-12 py-6 flex justify-between items-center z-50 mix-blend-difference ${className}`}>
        <div className="flex items-center gap-12">
          <TransitionLink href="/" className="text-3xl font-bold text-white relative z-50">
            JK
          </TransitionLink>
        ...
        {/* Desktop CTA */}
        <TransitionLink href="#contact" className="hidden lg:block text-white border border-white/30 px-6 py-2 rounded-full hover:bg-white hover:text-black transition-all">
          Let's Talk
        </TransitionLink>
      ...
      {/* Mobile Menu Overlay */}
      <div className={`fixed inset-0 bg-black z-40 transition-transform duration-500 ease-in-out lg:hidden flex flex-col items-center justify-center gap-8 ${isMobileMenuOpen ? 'translate-x-0' : 'translate-x-full'}`}>
        <TransitionLink 
          href="/gallery" 
